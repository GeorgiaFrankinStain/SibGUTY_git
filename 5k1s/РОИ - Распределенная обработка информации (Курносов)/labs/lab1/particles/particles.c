...
void distance_vec_f(float *x, float *y, float *z, float *d, int n)
{
	__m128 *xx = (__m128 *)x;
	__m128 *yy = (__m128 *)y;
	__m128 *zz = (__m128 *)z;
...
void distance_vec_d(double *x, double *y, double *z, double *d, int n)
{
...
void distance_vec_avx(float *x, float *y, float *z, float *d, int n)
{
    ...
    int number_items_processed_sse = number_packages_sse * number_float_in_m256;
	for (int i = number_items_processed_sse; i < n; i++)
		d[i] = sqrtf(x[i] * x[i] + y[i] * y[i] + z[i] * z[i]);
}
...
double run_scalar()
{
    ...
    printf("%.6f sec.   -   Elapsed time (scalar): \n", t);
    free(x);
    free(y);
    free(z);
    free(d);
...
double run_vectorized_f()
{
    ...
    printf("%.6f sec.   -   Elapsed time (vectorized SSE float): \n", t);
    free(x);
    free(y);
    free(z);
    free(d);
...
double run_vectorized_avx()
{
    ...
    printf("%.6f sec.  -  Elapsed time (vectorized AVX): \n", t);
    free(x);
    free(y);
    free(z);
    free(d);
...
int main(int argc, char **argv)
{
    ...
    double tvec_float = run_vectorized_f();
    double tvec_double = run_vectorized_d();
    double tvec_avx = run_vectorized_avx();
    printf("  Speedup (SSE float): %.2f\n", tscalar / tvec_float);
    printf(" Speedup (SSE double): %.2f\n", tscalar / tvec_double);
