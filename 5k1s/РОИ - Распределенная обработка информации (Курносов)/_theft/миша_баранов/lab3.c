...
double measure_allreduce(MPI_Comm comm)
{
	int rank, commsize;
	MPI_Comm_rank(comm, &rank);
	MPI_Comm_size(comm, &commsize);
	// Synchronize
	double clock_offset = sync_clock_linear(comm, 0);
	// bcast_time
	double bcast_time = measure_bcast_double(comm);
	MPI_Barrier(comm);
	int is_late = 1;
	...
	MPI_Bcast(&barrier_time, 1, MPI_DOUBLE, 0, comm);
	//Transfer gTime to lTime
	int is_local_late = 0;
	...
	if (t > barrier_time) {
		is_local_late = 1;
		fprintf(stderr, "Err: rank %d was late\n", rank);
	} else {
	...
	double stop_time = MPI_Wtime() - local_time;

	printf("process %3d: allreduce time: %.6f\n", rank, stop_time);
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < 10; ++i) {
		allreduce_times[i] = measure_allreduce(MPI_COMM_WORLD);
		MPI_Reduce(&allreduce_times[i], &max_times[i], 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
	...
	if (rank == 0 ) {
		...
		printf("average execution time of the Allreduce function: %.6f", average);
	}
